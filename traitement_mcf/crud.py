...
def get_rncp_code(rncpcode, session):
    """
...
def insert_rncp_code(rncpcode, libelle, session):
    """
    ...
    rncp = models.RNCP_Info(Code=rncpcode,Libelle=libelle)
    session.add(rncp)
    session.commit()
...
def get_formacode(formacode, session):
    """
...
def insert_formacode(formacode, libelle, session):
    """
...
def insert_formacode_table(formacode, idformation, session):
    """
    Insert new formacode infos into formacodes table

    Parameter(s)
        formacode           (int): formacode.
        item                (int): item to insert into  formacodes table.
        session         (Session): OPTIONAL. SQLAlchemy session to use. If one
                                    session is provided, then it is simply used
                                    but not closed. if no session is provided,
                                    then one is open and closed at the end.
...
def insert_nsf_table(nsfcode, idformation, session):
    """
...
def insert_rs_table(rscode, idformation, session):
    """
...
def insert_rncp_table(rncpcode, idformation, session):
    """
...
def get_rs_code(rscode, session):
    """
...
def insert_rs_code(rscode, libelle, date_fin, session):
    """
...
def get_nsf_code(nsfcode, session):
    """
...
def insert_nsf_code(nsfcode, libelle, session):
    """
...
def get_formation(idsimplon, session):
    """
...
def get_id_formation(idsession, session):
    """
...
def insert_formation(idsimplon, libelle, resumeprogramme, session, siretsimplon="79279132900032"):
    """
...
def insert_organisme_test(session):

    """Insert new organisme, Simplon with SIRET 79279132900032
    """
    siret_simplon = "79279132900032"
    query = (session
             .query(models.Organismes.Siret)
             .filter_by(Siret=siret_simplon)
             .first())
    if query:
        pass
    
    else:
        organisme = models.Organismes(Nom="Simplon", Siret=siret_simplon)
